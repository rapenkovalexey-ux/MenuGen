...
async def start_edit(call: CallbackQuery, state: FSMContext):
    ...
    lines = ["<b>Редактирование меню</b>\n\nВыберите день — введите его номер:\n"]
    for d in days:
        day_num = str(d.get("day", ""))
        day_label = d.get("date_label", "День " + day_num)
        lines.append("- " + day_label + ": введите <code>" + day_num + "</code>")
...
async def choose_day_handler(message: Message, state: FSMContext):
    ...
    for m in day.get("meals", []):
        ...
        lines.append("- " + meal_name + ": введите <code>" + meal_type + "</code>")

    await message.answer(
        "Приёмы пищи в день " + str(day_num) + ":\n\n" + "\n".join(lines),
        parse_mode="HTML"
    )
